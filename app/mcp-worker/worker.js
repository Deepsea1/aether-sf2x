...
// Source-of-truth fallback: when a verification_id isn't in the local KV cache
// (e.g. it was created via the API directly, or the 30-day cache expired), look
// the warrant up in the Aether warrantRegistry — the durable, signed registry
// that holds every persisted warrant. This keeps get_warrant / explain_verdict
// usable for ANY warrant via the API or MCP server, not just this session's.
async function fetchRegistryRecord(id, env) {
  const url = (env.AETHER_WARRANT_REGISTRY_URL || '').trim();
  if (!url) return null;
  try {
    const res = await fetch(url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'x-api-key': env.AETHER_API_KEY || '' },
      body: JSON.stringify({ verification_id: id }),
    });
    if (!res.ok) return null;
    const data = await res.json().catch(() => null);
    return data?.verified_warrant || null;
  } catch { return null; }
}
...
async function callExplainVerdict(args, env) {
  ...
  if (record) {
    return {
      ...
      certified: record.certified,
      certification: record.certification,
      premises_count: (record.premises || []).length,
      sources_count: (record.sources || []).length,
      created_at: record.created_at,
      source: 'cache',
    };
  }
  // Cache miss → pull from the durable warrant registry (source of truth).
  const w = await fetchRegistryRecord(id, env);
  if (!w) throw new Error(`verification ${id} not found (it may have expired or never existed)`);
  return {
    verification_id: id,
    warrant_id: w.warrant_id,
    verdict: w.validity_status,
    trust_score: w.lineage?.trust_score ?? null,
    certified: w.validity_status === 'valid',
    certification: w.validity_status,
    premises_count: w.premises_count,
    sources_count: w.sources_count,
...
async function callGetWarrant(args, env) {
  ...
  // Cache miss → return the full signed warrant from the durable registry:
  // premises, conclusion, lineage, expiry, signature, source snapshots.
  const w = await fetchRegistryRecord(id, env);
  if (!w) throw new Error(`warrant ${id} not found (it may have expired or never existed)`);
  return {
    verification_id: id,
    warrant_id: w.warrant_id,
    verdict: w.validity_status,
    ...
    signature_scheme: w.signature_scheme,
    premises: w.premises,
    ...
    authoritative_grounding: w.authoritative_grounding,
