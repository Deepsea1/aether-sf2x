...
// Admin view of NewsletterSubscriber records.
...
export default function Subscribers() {
  ...
  const load = useCallback(async () => {
    try {
      const list = await base44.entities.NewsletterSubscriber.list('-created_date', 500);
      setRows(list || []);
    } catch (e) { /* */ }
  ...
  return (
    <AppShell>
      <div className="max-w-3xl mx-auto space-y-6">
        <div>
          <div className="inline-flex items-center gap-2 text-[11px] uppercase tracking-[0.16em] text-emerald-400/80 mb-2"><Mail className="h-3.5 w-3.5" /> Newsletter Subscribers</div>
          <h1 className="font-heading text-xl font-semibold text-white">Weekly AI Hallucination Report — subscribers</h1>
          <p className="text-sm text-slate-500 mt-1.5">Everyone who signed up for the weekly digest, with source and status.</p>
        </div>

        {loading && <div className="flex justify-center py-20"><Loader2 className="h-6 w-6 text-slate-500 animate-spin" /></div>}
        {!loading && rows.length === 0 && (
          <div className="rounded-2xl border border-dashed border-white/10 p-16 text-center text-sm text-slate-500">No subscribers yet. The homepage form captures signups.</div>
        )}
