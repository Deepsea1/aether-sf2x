...
**Still open after P4:** the technical-docs capability card carries **no measured false-block rate**, so `release_gate.mode: enforcing` remains locked — correctly, per §18.2. The blocker is now purely operational: a clean negative-control run needs Base44 integration credits (exhausted until 2026-09-04). Everything upstream of that measurement is fixed and tested. Also open: `streamVerify` warrants carry no `service_mode_at_issuance` (SSE shape, deliberate); `verifyResponse` reads `GroundingDoc` through the sessionless request client, which returns nothing under that entity's owner-scoped read RLS — grounding context is silently dropped on keyed API calls (pre-existing, unrelated to the RLS restoration).
...
**P4 — Defensible** (≈ days 61–90) — **BUILT AND LIVE 2026-08-12**, one exit criterion outstanding: independence analysis v1 ✅ · semantic diff / display eligibility on proof pages + embeds ✅ · capability card v1 for technical-docs ✅ *mechanism* (the card, the entity, the public read, and the symmetric §18.2 gate are live and correctly **locked**) — ❌ *measurement* (no clean negative-control run exists; blocked on Base44 integration credits until 2026-09-04, so enforcement stays advisory) · drift indicators + breaker levels wired to service modes ✅ · dogfooding gate live in CI ✅ (advisory, needs the `AETHER_API_KEY` repo secret to execute) · benchmark program formalized — partial: the measurement plumbing is now correct end-to-end (§28) but unrun. *Exit status:* every §32 gate that does not depend on a measured false-block rate passes; gate 10 (capability cards block in **both** directions) waits on that one run.
