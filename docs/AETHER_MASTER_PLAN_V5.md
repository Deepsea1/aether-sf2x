...
**CORPUS RISK-STRATIFIED 2026-08-12 — and the answer was "both, but stratify first."** v1 carried no tier labels, so one aggregate was stamped into all four by-risk slots; worse, v1's TRUE claims are entirely general knowledge (boiling point, the Titanic, chromosome count), so the published `critical: 0.10` measured a population containing **zero critical claims**. Driving that aggregate down would have unlocked critical-tier hard-blocking on evidence with no critical claims in it.

**The resolution rule, now enforced in code:** with n TRUE claims the finest non-zero false-block rate is 1/n, so a 0.05 critical threshold is *unmeasurable* below n=20 — at v1's n=10 the tier could only ever read 0.0 or 0.1, unlock or fail, both artifacts of sample size. `capabilityCard` publishes a tier's rate **only** when its TRUE count can resolve that tier's threshold (critical n≥20, high n≥10); otherwise null with the arithmetic stated, which the gate treats as not-measured.

`corpus-v2-stratified.js` (76 items) adds real domain claims at the gated tiers — **critical**: medical, legal, safety (20 TRUE + 10 negatives); **high**: security, compliance, finance (10 TRUE + 6). v1 is imported *unedited* and tagged `moderate`. `risk_tier` is declared on `CorrelationAudit.items`, since undeclared fields are silently stripped on write — the same trap that made the card unmeasurable before; verified 76/76 persisted.

**Measured live** (`gate-0-v2-stratified`, `6a7c36eb75edea5255d77730`, 76 items, 0 errors):

| tier | false-block | false-pass | n (TRUE) |
|---|---|---|---|
| moderate | 0.0 (0 blocked) | 0.0 | 10 |
| high | 0.10 (1 blocked) | 0.0 | 10 |
| **critical** | **0.10 (2 blocked)** | 0.0 | **20** |

The critical figure is still 0.10 and still fails 0.05 — the same number as before, but it now **means** something: two real medical/legal/safety claims were wrongly blocked, rather than an aggregate borrowed from trivia. **Stratification did not unlock the gate; it made the refusal honest.**

**Next work, in order:** (1) Lower critical-tier false-block from 0.10 to ≤0.05 — now genuinely the only thing between advisory and enforcing, and known from evidence rather than assumed. Diagnose the two blocked critical claims first: if they are verifier-conservatism on well-sourced medical/legal facts, that is a grounding problem, not a threshold problem. **Do not tune the threshold to fit the number.** (2) The model-extractor half of §6.3's dual path, for cross-sentence and compound claims (`distinct_unit_rate` 0.5909 — ~40% of claims still share a verdict with another). (3) Author annotations + the `compiler_miss` regression signal (§6.3), neither of which exists yet. Everything upstream of that measurement is fixed and tested. Also open: `streamVerify` warrants carry no `service_mode_at_issuance` (SSE shape, deliberate); `verifyResponse` reads `GroundingDoc` through the sessionless request client, which returns nothing under that entity's owner-scoped read RLS — grounding context is silently dropped on keyed API calls (pre-existing, unrelated to the RLS restoration).
