...
**P4 — Defensible** (≈ days 61–90): independence analysis v1 · semantic diff / display eligibility on proof pages + extension · capability card v1 for technical-docs (enforcement mode unlocks only with measured false-block) · drift indicators + breaker levels wired to service modes · benchmark program formalized over the shipped arena/leaderboard · dogfooding gate live (Aether verifies its own docs in CI). *Exit:* the ship gates in §32 that govern claims-about-Aether all pass.
